...
# operating system:  Linux 3.8-2-686-pae i686 [ELF] (3.8.0)
# libc:	(6.17.0)
# binutils:	(222)
...
install:: 9wm
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  9wm $(DESTDIR)$(BINDIR)/9wm

all:: 9wm.$(MANNEWSUFFIX)

9wm.$(MANNEWSUFFIX): 9wm.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname 9wm` && \
	   $(LN) `basename 9wm.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < 9wm.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) 9wm.$(MANNEWSUFFIX)

install.man:: 9wm.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) 9wm.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/9wm.$(MANSUFFIX)
